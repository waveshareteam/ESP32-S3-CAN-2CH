...
static void WIFI_PrintAPIP(const char *prefix)
{
...
static void WIFI_OnEvent(WiFiEvent_t event, WiFiEventInfo_t info)
{
  if (event == ARDUINO_EVENT_WIFI_AP_STACONNECTED) {
    Serial.printf("External device connected to AP, MAC: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
...
void WIFI_Init()
{
  ...
  while(!WiFi.softAP(ssid, password)) {
    ...
    Serial.printf("Try setting up the WIFI again.\r\n");
  } 
  delay(100);  
